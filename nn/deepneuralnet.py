...
class Layer:
    ...
    def __init__(self,
        ...
        self.weights = self.rng.normal(loc=0.0,
                                       scale=0.1,
                                       size=(self.n_neurons_self, self.n_neurons_input))

        self.bias = np.zeros(n_neurons_self)
    ...
    def forward(self,prev_a):
        ...
        self.z = np.dot(prev_a, self.weights.T) + self.bias
...
class DeepNeuralNet:
    ...
    def forward(self, x):
        ...
        # the first set of activations is basically just the training data applied to the input layer
        # shape: (n_examples, n_features)
        self.inputLayer.a = prev_a = x
...
dnn_train(model, X_train, y_train, X_valid, y_valid, num_epochs=50, learning_rate=0.1)
